...
const require_browser = async(apiToken, browserZone)=>{
    const sessionKey = `${apiToken}-${browserZone}`;
    if (!sessions.has(sessionKey))
    {
        sessions.set(sessionKey, new Browser_session({
            cdp_endpoint: await calculate_cdp_endpoint(apiToken, browserZone),
        }));
    }
    return sessions.get(sessionKey);
};

const calculate_cdp_endpoint = async(apiToken, browserZone)=>{
    try {
        ...
        const password_response = await axios({
            url: `https://api.brightdata.com/zone/passwords?zone=${browserZone}`,
            method: 'GET',
            headers: {authorization: `Bearer ${apiToken}`},
        });
        const password = password_response.data.passwords[0];

        return `wss://brd-customer-${customer}-zone-${browserZone}:`
            +`${password}@brd.superproxy.io:9222`;
    } catch(e){
        if (e.response?.status===422)
            throw new Error(`Browser zone '${browserZone}' does not exist`);
...
let scraping_browser_navigate = {
    ...
    execute: async({url}, ctx)=>{
        const page = await (await require_browser(ctx.apiToken, ctx.browserZone)).get_page({url});
        try {
            await page.goto(url, {
                timeout: 120000,
...
let scraping_browser_go_back = {
    ...
    parameters: z.object({}),
    execute: async(ctx)=>{
        const page = await (await require_browser(ctx.apiToken, ctx.browserZone)).get_page();
        try {
            await page.goBack();
            return [
...
let scraping_browser_type = {
    ...
    execute: async({selector, text, submit},ctx)=>{
        const page = await (await require_browser(ctx.apiToken, ctx.browserZone)).get_page();
        try {
...
let scraping_browser_wait_for = {
    ...
    execute: async({selector, timeout},ctx)=>{
        const page = await (await require_browser(ctx.apiToken, ctx.browserZone)).get_page();
        try {
...
let scraping_browser_screenshot = {
    ...
    execute: async({full_page = false},ctx)=>{
        const page = await (await require_browser(ctx.apiToken, ctx.browserZone)).get_page();
        try {
...
//         await calculate_cdp_endpoint(apiToken, browserZone) : null;
// } catch(e){
